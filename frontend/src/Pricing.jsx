...
const TIERS = [
  ...
  },
  {
    ...
    features: ['Полная история сделок', 'PnL по дням и разбивка по TP/SL', 'AI-ассистент (50 вопросов/день)', { label: 'Telegram-алерты', soon: true }],
...
const FAQ = [
  { q: 'Это реальная торговля или бэктест?', a: 'Каждая сделка в истории — реальный вход и выход по актуальным ценам Bybit, не бэктест на исторических данных и не нарисованные цифры.' },
  { q: 'Как считается винрейт?', a: 'Доля закрытых сделок с положительным PnL от общего числа закрытых — учитываются все исходы (TP, стоп, таймаут, закрытие каналом), без исключений в нашу пользу.' },
  { q: 'Что если канал закроет сделку сам?', a: 'Мы отслеживаем такие сообщения в канале и закрываем позицию на сайте автоматически — статус обновится с меткой «Закрыт каналом», без ручных правок задним числом.' },
