...
```
backend/    FastAPI + торговая логика + SQLite + фоновый трекер (раз в 2 мин)
...
| `nfi_strategy.py` | Бэктест/трекер: детекторы входа, уровни TP/SL, momentum (`STRATEGY_MODE`). Live NFI-автопоиск отключён |
| `scanner.py` | Фоновый поток: трекинг открытых позиций, тики xsec/trend, прогрев снапшота Скринера (без автооткрытия NFI) |
...
1. **Momentum** (бэктест/paper, live-автопоиск отключён) — высокочастотная, входит на переходе ADX+EMA в тренд, выход по TP/SL/таймауту. **Backtest PF не заслуживает доверия** — менялся на 30-60% от сдвига таймаута на одну свечу. Единственный настоящий арбитр — дальран.
...
При старте сразу запускается фоновый трекер. Проверить: http://localhost:8000/api/stats.
