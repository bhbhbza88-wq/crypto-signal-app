...
COMETAPI_BASE_URL = (
    os.getenv("CLAUDE_BASE_URL") or os.getenv("COMETAPI_BASE_URL") or "https://api.cometapi.com"
).strip()
...
# CometAPI → claude-sonnet-5; прямой Anthropic → sonnet 4.5
_default_claude_model = "claude-sonnet-5" if COMETAPI_KEY else "claude-sonnet-4-5-20250929"
CLAUDE_MODEL = (os.getenv("CLAUDE_MODEL") or _default_claude_model).strip() or _default_claude_model


def claude_api_key() -> str:
    """CometAPI приоритетнее — дешевле / есть free tier."""
